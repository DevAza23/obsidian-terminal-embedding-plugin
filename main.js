...
__export(main_exports, {
  VIEW_TYPE_EMBEDDED_AI_TERMINAL: () => VIEW_TYPE_EMBEDDED_AI_TERMINAL,
  default: () => EmbeddedAiTerminalPlugin
});
...
var cachedNodePty = null;
function loadNodePty(plugin) {
  if (cachedNodePty) {
    return cachedNodePty;
  }
  const runtimeRequire = typeof require === "function" ? require : typeof window !== "undefined" && "require" in window ? window.require : void 0;
  if (!runtimeRequire) {
  ...
  for (const candidate of candidates) {
    try {
      cachedNodePty = runtimeRequire(candidate);
      return cachedNodePty;
    } catch (error) {
...
var EmbeddedTerminalSettingsTab = class extends import_obsidian2.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian2.Setting(containerEl).setHeading().setName("Shell");
    new import_obsidian2.Setting(containerEl).setName("Shell executable").setDesc("Executable launched inside each tab.").addText((text) => {
    ...
    new import_obsidian2.Setting(containerEl).setName("Shell args").setDesc("Passed to the shell on startup.").addText((text) => {
    ...
    new import_obsidian2.Setting(containerEl).setName("Default working directory").setDesc("Defaults to the vault root when empty.").addText((text) => {
    ...
    new import_obsidian2.Setting(containerEl).setName("Font size").setDesc("Terminal font size in pixels.").addSlider((slider) => {
    ...
    new import_obsidian2.Setting(containerEl).setName("Cursor blink").setDesc("Use a blinking cursor in xterm.").addToggle((toggle) => {
    ...
    new import_obsidian2.Setting(containerEl).setHeading().setName("Startup");
    new import_obsidian2.Setting(containerEl).setName("Startup commands").setDesc("Optional commands sent to every new tab before provider-specific commands.").addTextArea((text) => {
      text.inputEl.rows = 4;
    ...
    new import_obsidian2.Setting(containerEl).setHeading().setName("Provider Commands");
  ...
  renderCommandSetting(containerEl, label, key) {
    new import_obsidian2.Setting(containerEl).setName(label).setDesc("Command sent inside a new shell tab for this launcher.").addText((text) => {
...
function formatDiagnosticContext(context) {
  ...
  return Object.entries(context).filter(([, value]) => value.trim().length > 0).map(([key, value]) => `${key}: ${value}`).join("\n");
}
var DiagnosticsStore = class {
  ...
  record(input) {
    const detail = [input.detail, formatDiagnosticContext(input.context), input.error ? formatUnknownError(input.error) : ""].filter((part) => part && part.trim().length > 0).join("\n\n");
    this.diagnostics = [
...
// src/citation.ts
var FileCitationAutocomplete = class {
  constructor(app, term, writeToShell, containerEl) {
    this.app = app;
    this.term = term;
  ...
  updateResults() {
    ...
    if (!query) {
      this.results = files.slice().sort((left, right) => right.mtime - left.mtime).slice(0, 8).map((item) => item.file);
      this.scheduleRender();
      return;
    }
    const prefix = [];
...
function getTerminalTheme(element) {
...
// src/session.ts
function makeSessionLabel(profileId, id) {
  switch (profileId) {
    case "codex":
      return `Codex ${id}`;
    case "claude":
...
var TerminalSession = class {
  constructor(plugin, view, id, profileId, cwd, startupCommand) {
    this.plugin = plugin;
    this.view = view;
    this.id = id;
    ...
    try {
      ...
      this.ptyExitDisposable = this.ptyProcess.onExit(({ exitCode }) => {
        ...
        this.term.write(`\r
[process exited with code ${exitCode}]\r
`);
        if (exitCode !== 0) {
  ...
  installDropHandlers() {
    ...
    const show = () => dropzone.addClass("is-visible");
    const hide = () => dropzone.removeClass("is-visible");
...
// src/constants.ts
var VIEW_TYPE_EMBEDDED_AI_TERMINAL = "embedded-ai-terminal-view";
...
var TerminalView = class extends import_obsidian3.ItemView {
  ...
  async onOpen() {
    ...
    container.addEventListener("mousedown", (event) => {
      if (event.target.closest(".vin-terminal-tab-bar")) {
        return;
      }
  ...
  createSession(profileId, startupOverride) {
  ...
  startRename(tab, labelEl, session) {
    if (this.isRenaming) {
      return;
    }
  ...
  restoreState() {
    ...
    for (const saved of this.pendingState.sessions) {
      ...
      try {
        ...
        new import_obsidian3.Notice(`Failed to restore terminal tab "${saved.name}": ${message}`, 8e3);
        continue;
      }
  ...
  validateState(state) {
    ...
    const sessions = payload.sessions.filter((item) => {
      if (!item || typeof item !== "object") return false;
      const value = item;
      return typeof value.id === "number" && Number.isFinite(value.id) && typeof value.name === "string" && typeof value.cwd === "string" && (value.profileId === "shell" || value.profileId === "codex" || value.profileId === "claude" || value.profileId === "opencode" || value.profileId === "custom") && typeof value.startupCommand === "string";
    }).map((item) => ({
      id: item.id,
      name: item.name,
      cwd: item.cwd,
    ...
    const nextId = typeof payload.nextId === "number" && Number.isFinite(payload.nextId) && payload.nextId > 0 ? Math.floor(payload.nextId) : 1;
  ...
  renderEmptyState(title = "No active terminal session.", description = "Start a shell tab to attach a terminal.") {
...
var TerminalHelpModal = class extends import_obsidian3.Modal {
  onOpen() {
...
// src/main.ts
var EmbeddedAiTerminalPlugin = class extends import_obsidian4.Plugin {
  constructor() {
    super(...arguments);
  ...
  async onload() {
    ...
    this.addCommand({
      ...
      callback: () => {
        ...
        void this.sendPathsToActiveSession([nodePath2.join(getVaultBase(this.app), file.path)]);
      }
    });
  ...
  async withEnsuredView(action) {
    await this.openSideTerminal();
    this.withActiveView(action);
  }
  withActiveView(action) {
    const view = this.getActiveTerminalView();
    if (!view) {
      new import_obsidian4.Notice("Open the embedded terminal first.");
      return;
    }
    action(view);
  }
  ...
  async sendPathsToActiveSession(paths) {
...
0 && (module.exports = {
  VIEW_TYPE_EMBEDDED_AI_TERMINAL
});
